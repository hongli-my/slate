...
1. 启动 Slate（双击 app，或全局热键 `Cmd+Shift+E` 呼出）
2. 侧边栏「📁 打开文件夹」选择笔记仓库目录
3. 文件树点击打开文件，`Cmd+S` 保存，`Cmd+W` 关闭标签
...
打开 `.md` 文件后点「👁 预览」或按 `⌘P`，右侧实时渲染（GFM + 代码高亮 + TOC 大纲 + 标题折叠）。支持 40+ 语言语法高亮、代码折叠、多光标编辑。

### 双链与反链（`[[wikilink]]`）

基于 CodeMirror 6 自动补全 + 全库索引的笔记互链系统：

- **创建链接**：输入 `[[` 自动弹出全库笔记名补全（模糊匹配排序），选中插入 `[[笔记名]]`；也可手写。
- **跳转**：预览中点击蓝色 `[[链接]]` 跳转目标笔记；目标不存在（橙色虚线）时点击会新建同名笔记。
- **反向链接**：打开被其他笔记引用的笔记时，编辑器底部自动出现「🔗 反向链接」面板，列出所有引用来源，点击跳转。

### 知识图谱（`⌘6`）

把全库双链关系渲染为力导向图：

- **模式**：全库 / 局部（从当前笔记出发展开 2 层关联）
- **方向过滤**：全部 / 出向 / 入向 / 双向
- **交互**：拖拽节点、滚轮缩放、空白处拖拽平移、点击节点跳转笔记
- **刷新**：改动后点「刷新」重新布局

### 思维导图（`⌘7`）

把当前 Markdown 文档的标题层级（`#` / `##` / `###`…）渲染为思维导图，支持滚轮缩放、拖拽平移，工具栏「+ / − / 适应」调整缩放。

### AI 助手

编辑器侧边栏点「✨ AI 助手」打开右侧滑出的对话面板：

- **会话**：每个工作目录对应一个会话，切换文件夹自动新建/复用
- **发送**：输入问题后 `⌘Enter`（或 `Ctrl+Enter`）发送，也可点「发送」
- **选中文本提问**：先在笔记选中一段文字再提问，AI 会把选中内容作为上下文一并发送
- **中止**：生成中按钮变为「停止」，点击中止
- 回答以 Markdown 渲染（代码块、列表、表格）

> 依赖对话引擎 sidecar（`pi-bridge`）已随 app 启动；引擎未就绪时提示「对话引擎忙，请稍后重试」。

### 网页 / 截图转笔记

侧边栏顶部两个剪藏按钮（存到当前文件夹 `Clippings/`）：

- **📥 网页转笔记**：粘贴 URL → AI 抓取并整理成 Markdown
- **📷 截图转笔记**：框选屏幕区域 → 视觉模型识别整理成 Markdown

### 粘贴图片

在 Markdown 文件中 `Cmd+V` 粘贴图片，弹窗选择：

- **Base64 嵌入**：内联到文档（便携，但增大文件体积）
- **保存到文件夹**：写入当前工作目录，插入相对路径 `![](图片名)`
